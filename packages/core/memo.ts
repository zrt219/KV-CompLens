...
export function generateUnderwritingMemo(result: Omit<CompAnalysisResult, "memo">): string {
  ...
  const excludedLines = excluded.length
    ...
    : "- No lower-ranked homes remain outside the review set.";
  ...
  return [
    ...
    "Review Set",
    `${result.sourceScanSummary.recordsScanned} local demo records scanned; ${result.sourceScanSummary.candidatePoolCount} candidate homes ranked; ${result.sourceScanSummary.selectedCompCount} selected for review. ${result.sourceScanSummary.dataBoundaryNote}`,
    "Selected Homes",
    compLines,
    "Homes Not Selected",
    excludedLines,
    "Adjustment Summary",
    `Adjustments cover square footage, beds/baths, age, condition, parking, and lot size. Average total adjustment across the selected homes is ${formatCurrency(adjustmentAverage)}.`,
    "Review Summary",
    `The estimate combines the selected homes, their source quality, sale timing, and a buffer for homes that vary more widely. Effective sample size is ${result.valuation.effectiveSampleSize}. Evidence balance is ${Math.round(result.valuation.evidenceEntropy * 100)}%. Blend estimate is ${formatCurrency(result.valuation.modelFusion.finalEstimate)}.`,
    "Method Note",
    ...
    "Review the selected homes, confirm adjustment assumptions, verify any real-world source documents, and require analyst approval before relying on the memo in a lending file."
  ].join("\n\n");
}
